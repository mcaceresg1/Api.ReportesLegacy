...
export interface ClipperContratoLima {
  CabeceraContrato: CabeceraContrato;
   DetalleEspacio: DetalleEspacio[];
   DetalleAsistencia: DetalleAsistencia[];
   BeneficiarioContrato: BeneficiarioContrato[];
   Fallecido: Fallecido[];
  //  Resumen: Resumen[];
  //  ResumenCapital: ResumenCapital[];
}

  
  /******************TACNA*************/
  export interface ClipperContratoTacna {
    // CabeceraContrato: CabeceraContrato;
    // DetalleEspacios: DetalleEspacio[];
    // DetalleAsistencias: DetalleAsistencia[];
    // ComprobantesEmitidos: ComprobanteEmitido[];
    // BeneficiariosFirmaContrato: Beneficiario[];
...
export interface CabeceraContrato {
  ...
  telOficina?: string;              // T0.TE2CLI (opcional)
...
export interface DetalleAsistencia {
  concepto?: string;
  proced?: string;
  agenciaFuneraria?: string;
  consejero?: string;
  supervisor?: string;
  fechaPago?: string; // o Date
  formaPago?: string;
  tipoAfect?: string;
  estipFunerario?: number;
  aporteInicial?: number;
  estipMensual?: number;
  estado?: string;
  subDetalle?: SubDetalleAsistencia[];
  comprobantes?: ComprobantesEmitidos[];
  pagosCaja?: PagosCaja[];
}

export interface SubDetalleAsistencia {
  aceptante?: string;
  codigoLetra?: string;
  banco?: string;
  ...
  saldo?: number;
  ubicacion?: string;
  estipendoMensual?: number;
  totalAbonoEfectivo?: number;
  totalAbonado?: number;
  totalDescuento?: number;
}

export interface ComprobantesEmitidos {
  documento?: string;
  numero?: string;
  fecha?: string; // o Date
  moneda?: string;
...
export interface BeneficiarioContrato {
  beneficiario?: string;
  fechaNacimientoBeneficiario?: string; // o Date
  dniBeneficiario?: string;
  telefBeneficiario?: string;
  estadoCivilBeneficiario?: string;
  sexoBeneficiario?: string;
}

export interface Fallecido {
  nomFallecido?: string;
  fechaEntierro?: string; // o Date
  estadoCivilFallecido?: string;
  sexoFallecido?: string;
  plataformaFallecido?: string;
  sectorFallecido?: string;
  tipoEspacioFallecido?: string;
  nivelFallecido?: string;
  medidaFallecido?: string;
  agenciaFallecido?: string;
}
