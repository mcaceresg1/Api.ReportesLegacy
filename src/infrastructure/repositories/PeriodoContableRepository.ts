...
export class PeriodoContableRepository implements IPeriodoContableRepository {
  ...
  async generarReporte(filtros: FiltroPeriodoContable, page: number = 1, limit: number = 25): Promise<{
    success: boolean;
    ...
    try {
      ...
      for (const table of tablesToCheck) {
        ...
        if (!tableExists || (tableExists[0] as any).count === 0) {
          ...
          return {
            ...
            message: `Tabla ${schema}.${table} no existe`
          };
        }
      }
      ...
      if (filtros.saldosAntesCierre) {
        // Query de conteo para saldos antes del cierre
        countQuery = `
          ...
          saldos_a_fecha AS (
            ...
            SELECT 
                ...
                CTA.saldo_normal,
                CTA.descripcion,
                0 AS saldo_inicial_local, 0, 0, 0,
                0 AS saldo_inicial_dolar, 0, 0, 0,
                0 AS saldo_inicial_unidades, 0, 0, 0
        ...
        // Query de datos con paginación para saldos antes del cierre
        dataQuery = `
          WITH saldos_maximos AS (
            SELECT 
                B.centro_costo,
        ...
        // Query de conteo para saldos históricos
        countQuery = `
        ...
        // Query de datos con paginación para saldos históricos
        dataQuery = `
          WITH saldos_historicos AS (
              SELECT 
                  B.centro_costo,
      ...
      const [countResults] = await exactusSequelize.query(countQuery, { type: QueryTypes.SELECT });
      const totalRecords = (countResults as any).total || 0;

      // Ejecutar consulta de datos
      const results = await exactusSequelize.query(dataQuery, { type: QueryTypes.SELECT });
      
      const totalPages = Math.ceil(totalRecords / limit);

      return {
        success: true,
        data: results as PeriodoContable[],
        pagination: {
          page,
          limit,
          total: totalRecords,
      ...
      return {
        ...
        message: `Error al generar reporte de periodos contables: ${error}`
      };
    }
  }
}
